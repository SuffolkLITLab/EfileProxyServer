...
public class TylerCodeUpdater {
  ...
  public static boolean executeCommand(
    ...
    if (command.equalsIgnoreCase("downloadIndiv")) {
        ...
        try {
			return cu.downloadIndiv(args, (table, loc) -> makeUrl(endpoint, table, loc));
    ...
    try (CodeDatabaseAPI cd = cdSupplier.get()) {
      ...
      Function<String, List<CourtCodelistInfo>> codelistGetter = (location) -> {
          ...
          try {
            ...
            Function<CourtCodelistType, CourtCodelistInfo> ahh = (cc1) -> {
              return new CourtCodelistInfo(
              cc1.getECFElementName().getValue(),
              cc1.getCourtCodelistURI().getIdentificationID().getValue(), Ecf4Helper.parseDate(cc1.getEffectiveDate()));
            };
      ...
      var systemUrls = cd.systemTables().stream().collect(Collectors.toMap((t) -> t, (t) -> makeUrl(codesSite, t, "")));

      if (command.equalsIgnoreCase("replaceall")) {
        return cu.replaceAll(systemUrls, codelistGetter, cd);
      } else if (command.equalsIgnoreCase("replacesome")) {
        return cu.replaceSome(
            systemUrls, codelistGetter, cd, args.subList(1, args.size()));
      } else if (command.equalsIgnoreCase("refresh")) {
        return cu.updateAll(systemUrls, codelistGetter, cd);
      } else {
