...
public class TylerErrorCodes {
  ...
  public static Optional<Error> checkErrors(tyler.efm.latest.services.schema.common.ErrorType error) {
  ...
  public static Response mapTylerCodesToHttp(Optional<Error> maybeError, Supplier<Response> defaultRespFunc) {
    ...
    if (tylerToHttp.containsKey(error.code)) {
      return Response.status(tylerToHttp.get(error.code))
          .entity(error.text)
          .build();
    }
