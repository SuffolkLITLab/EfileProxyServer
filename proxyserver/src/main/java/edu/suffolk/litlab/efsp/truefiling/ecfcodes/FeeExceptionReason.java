package edu.suffolk.litlab.efsp.truefiling.ecfcodes;

import edu.suffolk.litlab.efsp.ecfcodes.CodeDatabaseUtils.Column;
import java.sql.Connection;
import java.sql.PreparedStatement;
import java.sql.SQLException;
import java.util.List;
import java.util.UUID;

// Is per court
public record FeeExceptionReason(
    UUID id,
    String description,
    // TODO: make this a URL? will that be too strict?
    String formLink) {

  public static final String TABLE_NAME = "truefiling_feeexceptionreason";
  public static final List<Column> columns =
      List.of(
          new Column("id", "uuid"),
          new Column("description", "text"),
          new Column("formlink", "text"));

  public static final String CREATE_QUERY =
      """
    CREATE TABLE truefiling_feeexceptionreason (
      "id" text PRIMARY KEY,
      "description" text,
      "formlink" text,
      "location" varchar(80),
      "jurisdiction" varchar(80)
    )
      """;

  public static final String INSERT_QUERY =
      """
    INSERT INTO truefiling_feeexceeptionreason (
      "id", "description", "formlink", "location", "jurisdiction"
    ) VALUES (
     ?, ?, ?, ?, ?
    )
    """;

  public static final String DELETE_FROM_QUERY =
      """
  DELETE FROM truefiling_feeexceptionreason WHERE jurisdiction=? AND location=?
      """;

  public static final String DELETE_ALL_COURTS_FROM_QUERY =
      """
  DELETE FROM truefiling_feeexceptionreason WHERE jurisdiction=?
      """;

  public static PreparedStatement prepQuery(Connection conn, String jurisStr, String location)
      throws SQLException {
    String query =
        """
      SELECT id, description, formlink
      FROM trufiling_feeexceptionreason
      WHERE jurisdiction=? and location=?
        """;
    PreparedStatement st = conn.prepareStatement(query);
    st.setString(1, jurisStr);
    st.setString(2, location);
    return st;
  }
}
