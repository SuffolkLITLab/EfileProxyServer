...
public class EcfCodesService extends CodesService {
  ...
  public enum DesiredResult { NAMES, COURT_COVERAGE; }
  @GET
  @Path("/filing_types")
  public Response searchFilingTypes(@QueryParam("search") String searchTerm, @QueryParam("result") DesiredResult desiredResult) throws SQLException {
    ...
    try (CodeDatabase cd = cdSupplier.get()) {
      return switch (desiredResult) {
        ...
        case DesiredResult.COURT_COVERAGE -> cors(Response.ok(cd.courtCoverageFilingType(searchTerm)));
      };
    }
  }
