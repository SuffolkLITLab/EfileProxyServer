...
public class FilingDocDocassembleJacksonDeserializerTest {
  ...
  public void oldStyleShouldLoadDocument() throws FilingError, IOException {
    ...
    JsonNode node =
        mapper.readTree(this.getClass().getResourceAsStream("/filingdocs/old_style_doc.json"));
    ...
    assertEquals(
        "6586", doc.get().getFilingAttachments().head().getDocumentTypeFormatStandardName());
  }
  ...
  public void oneAttachmentShouldLoadDocument() throws FilingError, IOException {
    ObjectMapper mapper = new ObjectMapper();
    JsonNode node =
        mapper.readTree(this.getClass().getResourceAsStream("/filingdocs/one_attachment.json"));
    doc = FilingDocDocassembleJacksonDeserializer.fromNode(node, varToPartyId, true, collector);
    assertTrue(doc.isPresent());
    assertEquals(doc.get().getFilingAttachments().length(), 1);
    assertEquals(
  ...
  public void oneEnabledOneDisabledShouldLoadOneAttachment() throws FilingError, IOException {
    ...
    JsonNode node =
        mapper.readTree(
            this.getClass().getResourceAsStream("/filingdocs/one_enabled_one_disabled.json"));
    doc = FilingDocDocassembleJacksonDeserializer.fromNode(node, varToPartyId, true, collector);
    assertTrue(doc.isPresent());
    assertEquals(doc.get().getFilingAttachments().length(), 1);
    assertEquals(
  ...
  public void noEnabledShouldFilingError() throws IOException {
    ...
    JsonNode node =
        mapper.readTree(
            this.getClass().getResourceAsStream("/filingdocs/fail_no_enabled_attachment.json"));
    try {
      doc = FilingDocDocassembleJacksonDeserializer.fromNode(node, varToPartyId, true, collector);
  ...
  public void shouldFallbackToParentDocIfNoEnabled() throws FilingError, IOException {
    ...
    JsonNode node =
        mapper.readTree(
            this.getClass().getResourceAsStream("/filingdocs/fallback_to_upper_doc.json"));
    doc = FilingDocDocassembleJacksonDeserializer.fromNode(node, varToPartyId, true, collector);
    assertTrue(doc.isPresent());
    assertEquals(doc.get().getFilingAttachments().length(), 1);
    assertEquals(
        "6586", doc.get().getFilingAttachments().head().getDocumentTypeFormatStandardName());
    assertEquals(
        "motion_to_stay_eviction.pdf", doc.get().getFilingAttachments().head().getFileName());
  }
  ...
  public void noDocTypesShouldBeEmptyStrings() throws IOException, FilingError {
    ...
    JsonNode node =
        mapper.readTree(
            this.getClass().getResourceAsStream("/filingdocs/fail_missing_doc_types.json"));
    doc = FilingDocDocassembleJacksonDeserializer.fromNode(node, varToPartyId, true, collector);
    assertTrue(doc.isPresent());
    assertEquals(doc.get().getFilingAttachments().length(), 1);
  ...
  public void twoAttachmentsShouldLoadTwoAttachments() throws FilingError, IOException {
    ObjectMapper mapper = new ObjectMapper();
    JsonNode node =
        mapper.readTree(this.getClass().getResourceAsStream("/filingdocs/two_attachments.json"));
    doc = FilingDocDocassembleJacksonDeserializer.fromNode(node, varToPartyId, true, collector);
    assertTrue(doc.isPresent());
    ...
    assertEquals(
        "6586", doc.get().getFilingAttachments().head().getDocumentTypeFormatStandardName());
    assertEquals(
        "motion-to-stay-eviction.pdf", doc.get().getFilingAttachments().head().getFileName());
