...
public class TylerEcfWsCallback implements FilingAssemblyMDEPort {
  ...
  private static String documentToStr(ReviewedDocumentType doc, CourtLocationInfo courtInfo) {
    ...
    if (doc instanceof tyler.ecf.extensions.common.ReviewedDocumentType tylerDoc) {
      docDesc = Ecf4Helper.getNonEmptyText(tylerDoc.getDocumentDescriptionText());
      if (tylerDoc.getDocumentBinary() != null) {
        fileName =
            Ecf4Helper.getNonEmptyText(tylerDoc.getDocumentBinary().getBinaryDescriptionText());
        ...
        for (DocumentRenditionType ren : tylerDoc.getDocumentRendition()) {
          if (ren.getDocumentBinary() != null) {
            fileName = Optional.of(ren.getDocumentBinary().getBinaryDescriptionText().getValue());
            fileName =
                Ecf4Helper.getNonEmptyText(ren.getDocumentBinary().getBinaryDescriptionText());
          }
        }
      }
      reviewComments = Ecf4Helper.getNonEmptyText(tylerDoc.getFilingReviewCommentsText());
      rejectedReason = Ecf4Helper.getNonEmptyText(tylerDoc.getRejectReasonText());
    } else {
      for (DocumentRenditionType ren : doc.getDocumentRendition()) {
        if (ren.getDocumentBinary() != null) {
          fileName = Ecf4Helper.getNonEmptyText(ren.getDocumentBinary().getBinaryDescriptionText());
        }
      }
    }
  ...
  public MessageReceiptMessageType notifyFilingReviewComplete(
    ...
    List<String> charges =
        payment.getAllowanceCharge().stream().map(TylerEcfWsCallback::chargeToStr).toList();
