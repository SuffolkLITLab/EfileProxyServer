...
import https.docs_oasis_open_org.legalxml_courtfiling.ns.v5_0wsdl.courtschedulingmde.CourtSchedulingMDE;
import https.docs_oasis_open_org.legalxml_courtfiling.ns.v5_0wsdl.courtschedulingmde.CourtSchedulingMDEService;
...
public class Ecf5Helper {
  ...
  public static long sizeMeasureAsBytes(MeasureType type) {
    long measureNum = 0;
    if (type.getMeasureValueAbstract().getValue() instanceof Decimal deci) {
      measureNum = deci.getValue().longValue();
    }
    // TODO(brycew): MeasureUnitAbstract can only be Length or Mass, nothing else. Why? Until then,
    // just assume bytes.
    return measureNum;
  }
  ...
  public static Optional<String> getNonEmptyText(gov.niem.release.niem.niem_core._4.TextType text) {
  ...
  public static Optional<String> getNonEmptyText(
      gov.niem.release.niem.proxy.xsd._4.NormalizedString str) {
    if (str != null && str.getValue() != null && !str.getValue().isBlank()) {
  ...
  public static Optional<CourtSchedulingMDE> setupSchedulingPort(
      CourtSchedulingMDEService schedulingFactory, String tylerToken) {
    var creds = TylerUserNamePassword.userCredsFromAuthorization(tylerToken);
    if (creds.isEmpty()) {
      log.warn("No creds from " + tylerToken + "?");
      return Optional.empty();
    ...
    var mde = schedulingFactory.getCourtSchedulingMDE();
    ServiceHelpers.setupServicePort((BindingProvider) mde, creds.get());
    return Optional.of(mde);
  }
