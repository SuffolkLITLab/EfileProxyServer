...
public class Ecfv5CaseTypeFactory {
  ...
  private final ecfv5.https.docs_oasis_open_org.legalxml_courtfiling.ns.v5_0.ecf.ObjectFactory oasisObjFac;
  private final ecfv5.gov.niem.release.niem.proxy.xsd._4.ObjectFactory proxyObjFac;
  private final ecfv5.https.docs_oasis_open_org.legalxml_courtfiling.ns.v5_0.civil.ObjectFactory oasisCivilObjFac;
  ...
  public Result<CaseType, FilingError> createCaseType(FilingInformation info, 
    ...
    for (FilingDoc doc : info.getFilings()) {
      ...
      doc.getMotionType().ifPresentOrElse(mot -> {
  ...
  private EntityType serializeCaseParticipant(Person per, FilingInformation info, 
    ...
    if (matchingType.isEmpty()) {
      List<String> partyTypeChoices = partyTypes.stream().map(p -> p.code).collect(Collectors.toList());
      InterviewVariable ptVar = collector.requestVar("party_type", "Legal role of the party", "choices", partyTypeChoices);
    ...
    if (per.isOrg()) {
      ...
      orgType.getOrganizationAugmentationPoint().add(oasisObjFac.createOrganizationAugmentation(orgAug));
