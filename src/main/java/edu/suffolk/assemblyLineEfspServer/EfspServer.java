...
public final class EfspServer {
	...
	public static void testUserManagement(IEfmUserService port) throws JAXBException {
		...
		System.out.println("Get new user: " + getNewUserResp.getError().getErrorCode() + " " + getNewUserResp.getError().getErrorText());
		...
		headersList.add(TylerUserNamePassword.MakeHeader(authRes.getEmail(), changePassResp.getPasswordHash())); 
		ctx.put(Header.HEADER_LIST, headersList);
